...
- Prophet prediction intervals are based on forward simulations of a piecewise linear model with fitted rate of breakpoints \textcite{taylor18}.
...
* In some situations, especially if combined with SARIMA methods, this can give very competititive forecast results \textcite{makridakis20}.
...
**For compatibility with the Pytorch `DataLoader`**, we create a custom `Dataset` capturing the seasonal relationship $(m=12)$. 
The LSTM input should look back at least 12-24 steps. 
...
- Chapter 10 of \textcite{huang22} discusses artificial neural network methods and introduces LSTM. 
...
- The LSTM section was drafted in collaboration with Gemini, edited in collaboration with Claude, and then revised after reading \textcite{lim21}.
