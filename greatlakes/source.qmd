...
* Long time series, or high-dimensional models, will be hard to investigate using particle filter methods without access to a reasonably powerful machine.

* Profile likelhood calculations also benefit from more compute cores than a laptop can offer.

* Greatlakes CPU calculations should be sufficient. JAX, and therefore `pypomp`, will use use a GPU if it is present and if you provide a CUDA-enabled version of `jaxlib`.

* You can experiment with GPU computing if the wait is not too long on greatlakes, or if your project becomes computationally ambitious.

* These notes focus on using greatlakes CPUs.
...
* Here, it is useful to have a Python version set up for CPU and a version set up for GPU. These will need different versions of `jaxlib`.
...
```bash
srun --nodes=1 --account=stats531w26s001_class \
...
* `531w16/greatlakes/test.sbat` contains a SLURM script to submit a batch job running the test in `test.py`.
...
* The output is in the `.out` file corresponding the job.

* It may be interesting to  study these run times, and compare greatlakes with your laptop, though the main goal here is just to practice running the code.
