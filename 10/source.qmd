...
Define LSTM as a PyTorch `nn.Module` and train with standard gradient descent.
...
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in dataloader:
        ...
        # batch_y: (batch, 1) -> keep as is for MSELoss
...
## Forecasting and Inverse Transformations

Generate 120-step forecast and reverse all transformations to get back to original scale.
...
# scaled_data has shape (N, 1), squeeze to (N,), take last lookback values
current_window = torch.FloatTensor(scaled_data[-lookback:].squeeze())
...
# 2. Inverse non-seasonal differencing (d=1)
# Need the last value of diff_seasonal to start cumsum
last_diff_seasonal = diff_seasonal.iloc[-1]
forecast_seasonal_diff = np.cumsum(forecast_diff) + last_diff_seasonal

# 3. Inverse seasonal differencing (D=1, m=12)
# Need the last 12 values of the original series
last_12_values = data.iloc[-12:].values
forecast_original = np.zeros(120)
for i in range(120):
    if i < 12:
        forecast_original[i] = forecast_seasonal_diff[i] + last_12_values[i]
    else:
        forecast_original[i] = forecast_seasonal_diff[i] + forecast_original[i - 12]
...
plt.figure(figsize=(12, 5))
plt.plot(time, huron_level, 'k-', label='Observed')
plt.plot(lstm_time, forecast_original, 'g-', label='LSTM Forecast')
plt.xlabel('Year'); plt.ylabel('Lake level (m)')
plt.legend()
plt.tight_layout(); plt.show()
```
