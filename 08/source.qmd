...
plt.semilogy(freqs1[1:], psd1[1:], 'k-', label='Unadjusted')
plt.semilogy(freqs2[1:], psd2[1:], 'r-', label='Seasonally adjusted')
...
#| fig-height: 4
plt.figure(figsize=(10, 5))
plt.semilogy(freqs_u1[1:], freq_response[1:], '-')
...
plt.figure(figsize=(10, 5))
plt.semilogy(freqs_smooth[1:], psd_smooth[1:] / psd_raw[1:], '-')
plt.xlim(0, 1.5)
plt.ylabel("Frequency ratio")
plt.xlabel("Frequency (cycles per year)")
...
plt.semilogy(freqs_zoom[1:], psd_zoom[1:], '-')
plt.xlim(0, 0.7)
...
- `scipy.ndimage.gaussian_filter1d` is a *Gaussian kernel smoother*.
