...
\begin{eqnarray}\nonumber
\mbox{[P3]} && \hspace{5mm}
f_{Y_n|X_{0:N},Y_{1:n-1},Y_{n+1:N}}(y_n\given x_{0:N},y_{1:n-1},y_{n+1:N})\\
\nonumber
&& = f_{Y_n|X_n}(y_n\given x_{n})
\end{eqnarray}
...
\begin{align}
\mbox{[P7]}\nonumber
\hspace{3mm} & f_{Y_{n:N}|X_{n}}(y_{n:N}\given x_n)= f_{Y_n|X_n}(y_n\given x_n)
f_{Y_{n+1:N}|X_{n}}(y_{n+1:N}\given x_n).
\\ \nonumber
\mbox{[P8]}\hspace{3mm}
& f_{Y_{n+1:N}|X_{n}}(y_{n+1:N}\given x_n)
\\ \nonumber
& \hspace{3mm} = \int f_{Y_{n+1:N}|X_{n+1}}(y_{n+1:N}\given x_{n+1}) \,
...
If an LG-POMP model is appropriate, you avoid Monte Carlo computations needed for inference in general nonlinear POMPs.
...
- A sequence of $d_X\times d_X$ covariances, $\{\boldsymbol{\Sigma}_{X,n}, n \in 0:N\}$,
- A sequence of $d_Y\times d_X$ matrices, $\matB_{1:N}$,
- A sequence of $d_Y\times d_Y$ covariances, $\{\boldsymbol{\Sigma}_{Y,n}, n \in 1:N\}$.
...
\\ \nonumber
\mbox{[LG2]}\hspace{10mm}  & Y_{n} &=& \matB_n X_n + \eta_n,  \eta_n\sim \normal[0,\boldsymbol{\Sigma}_{Y,n}].
\end{align}
...
We check that the ARMA equation matches the matrix equation
...
## Different POMPs can give the same model for $Y_{1:N}$  
...
\begin{align} \nonumber
\mbox{[BSM1]} \hspace{10mm} & Y_n = L_n + S_n + \epsilon_n
\\ \nonumber
\mbox{[BSM2]} \hspace{10mm} & L_{n} = L_{n-1} + T_{n-1} + \xi_n
\\ \nonumber
\mbox{[BSM3]} \hspace{10mm} & T_{n} = T_{n-1} + \zeta_n
\\ \nonumber
\mbox{[BSM4]}  \hspace{10mm} & S_{n} = -\sum_{k=1}^{11} S_{n-k} + \eta_n
\end{align}
...
- `statsmodels.tsa.statespace.structural.UnobservedComponents` documentation says it uses a "diffuse prior" for initial values. We could go through the source code to find out exactly what that means. 

- We should expect optimization difficulties, as we saw for ARMA. `StructTS` in R warns about this:
...
\begin{align}\nonumber
\mbox{[SS2]} \hspace{6mm}& X_n &=& 2X_{n-1}-X_{n-2} + \epsilon_n,   \epsilon_n\sim \iid\; \normal[0,\sigma^2/\lambda]
\\ \nonumber
\mbox{[SS3]}  \hspace{6mm}& Y_n &=& X_n + \eta_n,  \eta_n\sim \iid\; \normal[0,\sigma^2]
\end{align}
...
\begin{align} \nonumber
 ...
 \frac{-1}{2\sigma^2} \left[ \sum_{n=1}^N(y_n-x_n)^2 -\lambda  \sum_{n=3}^N(\Delta^2 x_n)^2\right] + C.
\end{align}
\normalsize
...
- Python `statsmodels` uses a Kalman filter to evaluate the likelihood of an ARMA model (or ARIMA, SARMA, SARIMA).
...
\\ \nonumber
\mbox{[KF2]} \hspace{12mm} & \Sigma_{X|Y} & = & \Sigma_X - \Sigma_{XY}\, \Sigma_Y^{-1}\, \Sigma_{YX}.
\end{align}
