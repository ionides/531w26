...
- For any data $y^*_{1:N}$ and any probabilistic model $f_{Y_{1:N}}(y_{1:N};\theta)$ we define the likelihood function to be
$$ L(\theta) = f_{Y_{1:N}}(y^*_{1:N};\theta).$$
...
\begin{eqnarray*} \hat\theta &=& \hat\theta(y^*_{1:N})\\
...
- Recall the duality between hypothesis tests and confidence intervals: $\theta^{\langle 0\rangle}$ is in a 95% confidence interval for $\theta$ if and only if we fail to reject a null hypothesis of $\theta=\theta^{\langle 0\rangle}$ at the 5% level.
...
- The Fisher information standard errors suggest a highly significant estimate of $\phi_2$. We investigate this using profile and bootstrap standard errors.

- First, we compute the profile likelihood confidence interval. To do this, we maximize the ARMA likelihood while fixing the AR2 coefficient at a range of values.

\footnotesize
```{python}
#| echo: true
#| label: profile_calculation
import shelve
K = 500
ar2_vals = np.linspace(0.2, 1.1, K)
with shelve.open('cache/profile') as cache:
    if "profile_loglik" not in cache:
        profile_loglik = np.zeros(K)
        for k in range(K):
            model = ARIMA(huron_level, order=(2,0,1),enforce_stationarity=False)
            with model.fix_params({'ar.L2': ar2_vals[k]}):
                results = model.fit()
...
plt.plot(ar2_vals, profile_loglik)
plt.xlabel('ar2')
...
**Question.** Interpret the profile likelihood plot for $\phi_2$.
...
```{python}
#| echo: false
theta_ar2 = np.array(theta_list)
plt.figure(figsize=(10, 4))
plt.hist(theta_ar2, bins=30, density=True)
plt.xlabel('ar2')
plt.ylabel('Density')
plt.tight_layout()
plt.show()
...
def fit_arma21(j):
    Y_j = arma_process.generate_sample(
        ...
        scale=sigma) + intercept
    model_j = ARIMA(Y_j, order=(2,0,1)).fit()
    return model_j.params[2]  # AR2 coefficient
...
#| echo: false
#| eval: true
ar2_sim = [x for x in huron_sim if x is not None]
plt.figure(figsize=(10, 4))
plt.hist(ar2_sim, bins=50)
plt.xlabel('ar2')
...
- When the true model is AR(1) and we fit ARMA(2,1), it seems that we often obtain a model with estimated AR2 coefficient similar to what we estimated from data.

- Thus, we cannot reject an AR(1) hypothesis for the Huron data, even though the Fisher information based analysis appears to give strong evidence that the data should be modeled with a nonzero AR2 coefficient.
...
- Section 3.5 of \textcite{shumway17} and Section 4.2 of \textcite{huang22} give complementary discussions of parameter estimation for ARMA models.

- Section 3.7 of \textcite{shumway17} takes a different perspective on selecting ARMA models, putting less emphasis on likelihood. Section 4.3 of \textcite{huang22} is closer to the approach in these notes. Both perspectives can be valuable.
