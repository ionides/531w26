...
RL = 1   # run level 0: quick, 1: medium, 2: slow
...
* ARCH and GARCH models are widely used for financial time series modeling. Our introduction follows @cowpertwait09; see also Section 5.4 of @shumway17.
...
For optimization procedures such as iterated filtering, it is convenient to transform parameters to be defined on the whole real line. We therefore transform $\sigma_\eta$, $\sigma_\nu$ and $\phi$.
```{python}
#| echo: true
...
```{python}
#| echo: false
#| label: build test parameter vector
params_test = {
    "sigma_nu": np.exp(-4.5),
    "mu_h": -0.25,
...
fig, ax = plt.subplots(figsize=(8, 3))
sim_y = np.array(Y_sim["obs_0"]).flatten()
...
Repeated stochastic maximizations can show the geometry of the likelihood surface.
...
Optimization attempts from diverse remote starting points can approach our MLE, but do not exceed it. This gives us some reasonable confidence in our MLE.
...
```{python}
#| echo: false
#| label: read results for benchmark comparison
if 'r_box' not in dir():
    r_box = pickle.load(
