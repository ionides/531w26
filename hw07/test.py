...
def get_random_normal(key: jax.Array, shape: tuple[int, ...]):
...
# VMAP over JITted function (cached)
keys4 = jax.random.split(key2, 10)
keys4_new, x4 = jax.vmap(lambda k: get_random_normal_jit(k, shape=(int(N / 10),)))(
    keys4
)
x4.block_until_ready()
end4_time = time.perf_counter()

print(
    f"time0: {end0_time - start_time:.3f}s, ",
    f"time1: {end1_time - end0_time:.3f}s, ",
    f"time2: {end2_time - end1_time:.3f}s, ",
    f"time3: {end3_time - end2_time:.3f}s, ",
    f"time4: {end4_time - end3_time:.3f}s",
)
