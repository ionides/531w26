...
However, a more careful optimization using multiple starting values (e.g., the R package `arima2::arima`) shows us that ARMA(2,1) actually has a higher AIC than ARMA(2,2) so all lines of evidence suggest ARMA(2,1) is a better choice. The differences are small, so the choice is unlikely to be highly consequential.
