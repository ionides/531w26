...
This table was produced in R with the widely used `tseries::logLik.garch` function.
...
(E). `tseries::logLik.garch` produces something that is not the log-likelihood of $y_{1:N}$, so we cannot readily compare it between models.
